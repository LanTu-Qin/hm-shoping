    ...
    <van-action-sheet v-model="show" :title="mode === 'cart' ? '加入购物车' : '立刻购买'">
      <div class="product">
        ...
        <div class="showbtn" v-if="detail.stock_total>0">
          <div class="btn" v-if="mode==='cart'?true:false" @click="addCart">加入购物车</div>
...
<script>
import { getEvaluation, getProductDetail } from '@/api/product'
import CountBox from '@/components/CountBox.vue'
import loginconfirm from '@/mixins/loginconfirm'
import { Toast } from 'vant'
import { addCart, getCartNum } from '@/api/cart'
...
export default {
  ...
  data () {
    return {
      ...
      commentList: [],
      totalcomment: 0,
      defaultImage: 'https://smart-shop.itheima.net/static/default-avatar.png',
      show: false,
      mode: 'cart',
      addcount: 1,
      cartTotal: 0
  ...
  methods: {
    ...
    async getComment () {
      const { data: { list, total } } = await getEvaluation(this.goodsId, 5)
      this.commentList = list
      this.totalcomment = total
      // console.log(this.commentList, this.totalcomment)
    },
    async getCartTotal () {
      const { data: { cartTotal } } = await getCartNum()
    ...
    async addCart () {
      if (this.loginConfirm() === false) return
      const { data: { cartTotal } } = await addCart(
        this.goodsId,
        this.addcount,
